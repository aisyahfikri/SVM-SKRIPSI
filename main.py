...
tfidf_vectorizer = joblib.load("script_deploy/tfidf_vectorizer.pkl")
svm_model = joblib.load("script_deploy/svm_model.pkl")
label_encoder = joblib.load("script_deploy/label_encoder.pkl")
...
def get_data():
    ...
    # Total data dari tabel klasifikasi
    cursor.execute("SELECT COUNT(*) as total FROM klasifikasi")
    total = cursor.fetchone()["total"]

    # Total data positif berdasarkan label_prediksi di klasifikasi
    cursor.execute("SELECT COUNT(*) as positif FROM klasifikasi WHERE label_prediksi='positif'")
    positif = cursor.fetchone()["positif"]

    # Total data negatif berdasarkan label_prediksi di klasifikasi
    cursor.execute("SELECT COUNT(*) as negatif FROM klasifikasi WHERE label_prediksi='negatif'")
